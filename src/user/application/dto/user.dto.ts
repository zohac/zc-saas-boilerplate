...
export class UserDto {
  @ApiProperty({description: 'User unique identifier (UUID)', example: 'a1b2c3d4-...'})
  id: string;
  ...
  @ApiProperty({description: "User's phone number", example: '+33612345678', required: false})
  telephone: string | null;

  @ApiProperty({description: "URL of the user's profile image", required: false})
  imageUrl: string | null;

  @ApiProperty({description: 'Indicates if the user account is active', example: true, required: false})
  isActive: boolean;

  @ApiProperty({description: 'Timestamp of user creation', required: false})
  createdAt: Date;

  @ApiProperty({description: 'Timestamp of last user update', required: false})
  updatedAt: Date;
